...
if __name__ == '__main__':
    ...
    with open(args.config, 'r') as f: #open the jsonified model from the parameter config
        ...
        experiment_configs = config_file['experiment_configs']  #get the configurations for the model, this will be an array
    ...
    for i, experiment_config in enumerate(experiment_configs):
        ...
        experiment_type = get_config_default_value_if_none(experiment_config, 'type') #get the type associated with the experiment
        if experiment_type == 'none': # skip this model configuration if this has a none type
            continue
            
        if get_config_default_value_if_none(experiment_config, 'previous_config_offset') == 0: #check if there was a previous configuration 0 means there was none
            ...
            previous_config = experiment_configs[i - experiment_config['previous_config_offset']] # access the previous config from the configs by using the offset to subtract it from i
            # if verbose > 0:
        ...
        initial_learning_rate = get_config_default_value_if_none(experiment_config, 'initial_learning_rate') # get the learning rate for the model from the config
        epochs = get_config_default_value_if_none(experiment_config, 'epochs') # get the number of epochs from the config
        batch_size = get_config_default_value_if_none(experiment_config, 'batch_size') # get the batch size form the configuration
        optimizer_type = get_config_default_value_if_none(experiment_config, 'optimizer') # get the optimizer from the config
        ...
        if experiment_type == 'transform_train': # if this is a transform train experiment NOT APPLICABLE FOR BASE MODEL this would be used for the transformation training described in the overall structure
            ...
            if 'unlabelled' not in prepared_datasets: # if there is not a unlablled dataset
                prepared_datasets = load_unlabelled_dataset(prepared_datasets, args.unlabelled_dataset_path, window_size, labelled_repeat, max_unlabelled_windows=args.max_unlabelled_windows, verbose=verbose) # create the unlabelled data

            if previous_config is None or get_config_default_value_if_none(previous_config, 'trained_model_path', set_value=False) == '': # if there was not a previous model or there is not a stored model
                ...
                core_model = self_har_models.create_1d_conv_core_model(input_shape) #create a new model that is a 1D convolutional  model
                """
            ...
            multitask_transform_dataset = self_har_utilities.create_individual_transform_dataset(prepared_datasets['unlabelled_combined'], transform_funcs_vectorized) # this will add the unlabelled combined with the labelled with the labels removed

            multitask_transform_train = (multitask_transform_dataset[0], self_har_utilities.map_multitask_y(multitask_transform_dataset[1], transform_funcs_names)) # get the multitask inputs
            multitask_split = self_har_utilities.multitask_train_test_split(multitask_transform_train, test_size=0.10, random_seed=42) # create the split
            multitask_train = (multitask_split[0], multitask_split[1]) #training
            multitask_val = (multitask_split[2], multitask_split[3]) # validation


            def training_rate_schedule(epoch): # this is the schedule of which the model learning should be updated  per batch
        ...
        if experiment_type == 'har_full_train' or experiment_type == 'har_full_fine_tune' or experiment_type == 'har_linear_train': # if the model is a full train (what the default is)
            ...
            is_core_model = False
            if previous_config is None or get_config_default_value_if_none(previous_config, 'trained_model_path', set_value=False) == '': # if this is the first model being trained, no previous models to use
                if verbose > 0:
                    print("Creating new model...")
