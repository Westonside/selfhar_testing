...
def prepare_dataset(dataset_path, window_size, get_train_test_users, validation_split_proportion=0.1, verbose=1):
    ...
    with open(dataset_path, 'rb') as f:  # open the dataset path
        ...
        user_datasets = dataset_dict[
            'user_split']  # this is where you get the user the users associated with all their activityies from the loadded model format: {1:[[([[.. associates a user with a series that has a label
        label_list = dataset_dict[
            'label_list']  # There are the types of activities being performed, the labels will be 1D list [sit, std, wlk,ups,...

    label_map = dict([(l, i) for i, l in
                      enumerate(label_list)])  # This will create a dictionary of a the activitiy name to integer ids
    output_shape = len(label_list)  # The number of output values found by just getting number of labels

    har_users = list(
        user_datasets.keys())  # Get the key value in the user dict to get all the numeric user ids then turn that into a 1D list
    train_users, test_users = get_train_test_users(
        har_users)  # get the split of training and testing users using the passed in function this will do an 80-20 split where 1 in 5 users will be in the testing set
    ...
    np_train, np_val, np_test = data_pre_processing.pre_process_dataset_composite(
        ...
        shift=window_size // 2,
        # divide the window into two and divide down this will be how much the window moves each time so 200 in the case of 400 meaning 50% intersect
        normalise_dataset=True,
        validation_split_proportion=validation_split_proportion,
        # The validation set is used to determine how good our training set is performing it is .1 by default
        verbose=verbose
    )
    ...
    return {
        ...
        # this will be the #this will be the shape of inputs 400 rows with 3 cols each
        'output_shape': output_shape,
    }
...
def load_unlabelled_dataset(prepared_datasets, unlabelled_dataset_path, window_size, labelled_repeat,
                            max_unlabelled_windows=None, verbose=1):
    def get_empty_test_users(har_users):
        return (har_users, [])

    prepared_datasets['unlabelled'] = \
    prepare_dataset(unlabelled_dataset_path, window_size, get_empty_test_users, validation_split_proportion=0,
    ...
    prepared_datasets = {
        ...
        **generate_unlabelled_datasets_variations(
            # manually generates unlabelled and combined with the lablled training data
            prepared_datasets['unlabelled'],
...
if __name__ == '__main__':
    ...
    if use_tensor_board_logging:  # check if tensorboard logging is enabled
        logs_directory = os.path.join(working_directory,
                                      LOGS_SUB_DIRECTORY)  # combine the current wd with the logs sub directory name
    ...
    models_directory = os.path.join(working_directory,
                                    MODELS_SUB_DIRECTORY)  # creating relative path to wehre the models are stored
    ...
    transform_funcs_vectorized = [
        ...
        # this creates an arraye of the transformation functions
    ...
    def get_fixed_split_users(har_users):  # TODO: improve flexibility
        test_users = har_users[
                     0::5]  # the test users are every 5 users [0,5,10,15,...] so 0 till the end jump by 5 these will go into the testing set so 1/5 = 20% user split
        train_users = [u for u in har_users if
                       u not in test_users]  # the training users are everything not ine the testing set 80%
    ...
    prepared_datasets['labelled'] = prepare_dataset(wisdm, window_size, get_fixed_split_users,
                                                    validation_split_proportion=0.1, verbose=verbose)
    ...
    for i, experiment_config in enumerate(experiment_configs):
        ...
        if experiment_type == 'har_full_train' or experiment_type == 'har_full_fine_tune' or experiment_type == 'har_linear_train':  # if the model is a full train (what the default is)
            ...
            if previous_config is None or get_config_default_value_if_none(previous_config, 'trained_model_path',
                ...
                core_model = self_har_models.create_1d_conv_core_model(
                    input_shape)  # create the convolution model that has input shape (400,3) this is the core model and everything else will go on top
                ...
                if experiment_type == 'har_linear_train':  # after you have evaluated the student model
                    core_model = self_har_models.extract_core_model(
                        previous_model)  # get the core cnn model from the past stage of student fine tunining this will be the core CNN model
                    is_core_model = True
                    ...
                    'trained_model_type'] == 'transform_with_har_model':  # if your model has been trained on the transformation data
                    har_model = self_har_models.extract_har_model(previous_model, optimizer=optimizer,
                                                                  model_name=tag)  # this will create a model that has the same inputs as the last model but the outputs will be only the HAR head's
                    is_core_model = False
                else:
            ...
            if is_core_model:  # if you have a core model
                if experiment_type == 'har_linear_train':  # after the fine tuning stage
                    self_har_models.set_freeze_layers(core_model,
                                                      num_freeze_layer_index=None)  # freeze all the layers from the core model
                    har_model = self_har_models.attach_linear_classification_head(core_model, output_shape,
                                                                                  optimizer=optimizer,
                                                                                  model_name="Linear")  # add a linear classifier this allows testing the performance of trained core model

                elif experiment_type == 'har_full_train':
                    self_har_models.set_freeze_layers(core_model,
                                                      num_freeze_layer_index=0)  # say if some layers can be trained or not this enables all layers to train and learn
                    # for the case of the teacher model you don't freeze any layers
                    har_model = self_har_models.attach_full_har_classification_head(core_model, output_shape,
                                                                                    optimizer=optimizer, num_units=1024,
                                                                                    model_name="HAR")  # add the classifier to the end of the model it has 6 outputs in 1D
                ...
                if experiment_type == 'har_full_train':
                    self_har_models.set_freeze_layers(self_har_models.extract_core_model(har_model),
                                                      num_freeze_layer_index=0)
                elif experiment_type == 'har_full_fine_tune':  # if you are about to fine tune the student mode
                    self_har_models.set_freeze_layers(self_har_models.extract_core_model(har_model),
                                                      num_freeze_layer_index=5)  # this takes the functional layer that comes after the iunput layer and freeze the 5th layer


            def training_rate_schedule(
                    epoch):  # define the training rate schedule while is setting to the starting rate
            ...
            training_schedule_callback = tf.keras.callbacks.LearningRateScheduler(
                training_rate_schedule)  # this creates the training schedule
            # TRAINING START
            ...
            best_har_model_file_name, last_har_model_file_name = self_har_trainers.composite_train_model(
                ...
                working_directory=working_directory,
                callbacks=[training_schedule_callback],
                # in the case of the linear train (after the model has been fine tuned, this freezes the layers and creates a linear classifier this serves as a final eval
                epochs=epochs,
                batch_size=batch_size,
                tag=tag,
            ...
            experiment_config[
                'trained_model_path'] = best_har_model_file_name  # set the file path for the best configuration model
        ...
        if experiment_type == 'self_training' or experiment_type == 'self_har':  # this will not be run for the teacher model
            if 'unlabelled' not in prepared_datasets:  # If you do not have the unlablled dataset prepared
                prepared_datasets = load_unlabelled_dataset(prepared_datasets, args.unlabelled_dataset_path,
                                                            window_size, labelled_repeat,
                                                            max_unlabelled_windows=args.max_unlabelled_windows)  # load  the unlabelled data
                # load in the unlablled data and combine it with the training data without labels add it to the datasets dicts
            if previous_config is None or get_config_default_value_if_none(previous_config, 'trained_model_path',
                                                                           set_value=False) == '':  # if the previous config does not exist or the config was not saved give up
                print("ERROR No previous model for self-training")  # error case
                break  # stop
            else:
                if verbose > 0:  # printing
                    print(f"Loading previous model {previous_config['trained_model_path']}")
                teacher_model = tf.keras.models.load_model(previous_config[
                                                               'trained_model_path'])  # load the previously trained teacher set that was trained on training data
            ...
            np_self_labelled = self_har_utilities.pick_top_samples_per_class_np(
                # this will pick the predictions with the highest confidence from the teacher for each class
            ...
            def training_rate_schedule(
                    epoch):  # define a trainng rate schedule that updates and goes down with the epoch
            ...
            training_schedule_callback = tf.keras.callbacks.LearningRateScheduler(
                training_rate_schedule)  # create a learning rate scheduler

            if experiment_type == 'self_training':
                student_pre_train_dataset = np_self_labelled

                student_model = self_har_models.attach_full_har_classification_head(core_model, output_shape,
                                                                                    optimizer=optimizer,
                                                                                    model_name="StudentPreTrain")
                student_model.summary()

                pre_train_split = sklearn.model_selection.train_test_split(student_pre_train_dataset[0],
                                                                           student_pre_train_dataset[1], test_size=0.10,
                                                                           random_state=42)
                ...
                multitask_transform_y_mapped = self_har_utilities.map_multitask_y(multitask_transform_y,
                                                                                  transform_funcs_names)  # using the one hot encoding of the transformations applied to each x create a map
                multitask_transform_y_mapped[
                    'har'] = multitask_har_y  # set the mapping for har to be the teacher's probabilities
                self_har_train = (multitask_X,
                                  multitask_transform_y_mapped)  # the training set is the transformed data along with the mappings of what transformation was applied
                ...
                student_model = self_har_models.attach_multitask_transform_head(core_model,
                                                                                ...
                                                                                num_units_har=1024,
                                                                                model_name="StudentPreTrain")  # add the multitask learning head now to the core model for the student
                student_model.summary()
                # create the treaining split
                pre_train_split = self_har_utilities.multitask_train_test_split(student_pre_train_dataset,
                                                                                test_size=0.10,
                                                                                random_seed=42)  # create a split in the training set that is used for training and validation

                student_pre_train_split_train = (
                pre_train_split[0], pre_train_split[1])  # get the training data and labels
                student_pre_train_split_val = (
                pre_train_split[2], pre_train_split[3])  # create the validation set split
            ...
            best_student_pre_train_file_name, last_student_pre_train_file_name = self_har_trainers.composite_train_model(
                ...
                working_directory=working_directory,
                callbacks=[training_schedule_callback],
                # this will train the student on the teacher labelled data that has been transformed
                epochs=epochs,
                batch_size=batch_size,
                tag=tag,
                use_tensor_board_logging=use_tensor_board_logging,
            ...
            experiment_config[
                'trained_model_path'] = best_student_pre_train_file_name  # put the now trained stuednt model filepath in the dictionary
            if experiment_type == 'self_training':
                experiment_config['trained_model_type'] = 'har_model'
            else:
                experiment_config[
                    'trained_model_type'] = 'transform_with_har_model'  # tset the trained model type to be the model trained on transformation and har
        ...
        if get_config_default_value_if_none(experiment_config, 'eval_har', set_value=False):  # the evaluation stage
            if get_config_default_value_if_none(experiment_config, 'trained_model_type') == 'har_model':
                best_har_model = tf.keras.models.load_model(
                    experiment_config['trained_model_path'])  # load the fine-tuned student model
            elif get_config_default_value_if_none(experiment_config,
                                                  'trained_model_type') == 'transform_with_har_model':
            ...
            pred = best_har_model.predict(prepared_datasets['labelled']['test'][
                                              0])  # have the fine tuned student model predit on the labelled testing dasta  EVALUATION or if the linear train
            eval_results = self_har_utilities.evaluate_model_simple(pred, prepared_datasets['labelled']['test'][
                1])  # evaluate the model accuracy using the labels to see how accurate
            if verbose > 0:
                print(eval_results)
