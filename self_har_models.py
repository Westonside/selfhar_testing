...
def create_1d_conv_core_model(input_shape, multi_modalities = None, model_name="base_model", use_standard_max_pooling=False):
    """
    ...
    if multi_modalities is not None:
        ...
        inputs = tf.keras.Input(shape=input_shape, name='input') #create the input leayer which takes a windowed size amount of data (400,3)
    x = inputs # put the input layer
    x = tf.keras.layers.Conv1D( #the first param is the number of filers, 32 filters with a kernel size of 24  you are adding now a convolutional layer
            32, 24,
            activation='relu',
            kernel_regularizer=tf.keras.regularizers.l2(l=1e-4) #regularizers allow applying a penalty to the layer's kernel these penatlties are summed into the loss function and these penalties are applied on a per layer basis
        )(x)
    x = tf.keras.layers.Dropout(0.1)(x) # now have a dropout layer where there is a 10% dropout to input layers, avoiding overfitting only applied when training is true
    ...
    if use_standard_max_pooling:
        ...
        x = tf.keras.layers.Reshape([x.shape[-1]], name='reshape_squeeze')(x) # the ouput of the 1d max pooling will be generally 3D so this will reshapoe the tensor to be [x.shape[-1]] specifies the target shape which would be to 1D last dimension of the shape
    else:
        x = tf.keras.layers.GlobalMaxPool1D(data_format='channels_last', name='global_max_pooling1d')(x) # add a global max pooling layer
...
def extract_har_model(multitask_model, optimizer, output_index=-1, model_name="har"):
    model = tf.keras.Model(inputs=multitask_model.inputs, outputs=multitask_model.outputs[output_index], name=model_name) # if the output index is -1 then you will get the HAR classifier output and ignore the other transformation classifiers
...
def set_freeze_layers(model, num_freeze_layer_index=None):
    if num_freeze_layer_index is None: #for the fine tune this takes the trained early conv layers are to be frozen
        ...
        for layer in model.layers[:num_freeze_layer_index]: #freeze up to the freeze laye (only freeze first 2 conv layers), this allows the model to fine tune to the final task dataset while retaining previously learned knowledge
            layer.trainable = False # disable the layer you are freezing the first 2 convolution layers
        for layer in model.layers[num_freeze_layer_index:]: # keep the other layers after the frozen ones trainable so this will be one convolutional layer
...
def attach_full_har_classification_head(core_model, output_shape, optimizer=tf.keras.optimizers.Adam(learning_rate=0.001), num_units=1024, model_name="HAR"):
    ...
    inputs = tf.keras.Input(shape=core_model.input.shape[1:], name='input') #create the input layer to the classifier this will expect (400, 3) take inputs from the layer before
    ...
    outputs = tf.keras.layers.Softmax()(x) # softmax will distribute the values of the vector to a proability distribution

    model = tf.keras.Model(inputs=inputs, outputs=outputs, name=model_name) #create a model with those inputs and those outputs the ouput being the probabilities
...
def attach_linear_classification_head(core_model, output_shape, optimizer=tf.keras.optimizers.SGD(learning_rate=0.03), model_name="Linear"):
    ...
    inputs = tf.keras.Input(shape=core_model.input.shape[1:], name='input') # this will create an input layer of shape (400,3) you do the [1:] to skip the batch will generally be (None, 400,3)
    ...
    x = tf.keras.layers.Dense(output_shape, kernel_initializer=tf.random_normal_initializer(stddev=.01))(intermediate_x) # add a dense layer with a random kernel init giving random weights to start with with a std of 0.01 and this will take the inputs of the input layer
...
def attach_multitask_transform_head(core_model, output_tasks, optimizer, with_har_head=False, har_output_shape=None, num_units_har=1024, model_name="multitask_transform"):
    ...
    inputs = tf.keras.Input(shape=core_model.input.shape[1:], name='input') # set an input layer coming from the base inputs that has a defined in the base convolutional layer
    ...
    if with_har_head: # if there is a HAR head there should only be one har head this will predict the human activity
        x = tf.keras.layers.Dense(num_units_har, activation='relu')(intermediate_x) # add a dense layer connected to the intermediate layer
        x = tf.keras.layers.Dense(har_output_shape)(x) # add another dense layer with the har output shape
        har_pred = tf.keras.layers.Softmax(name='har')(x) # apply the softamx which iwll convert a vector of numbers into a probability distribution  this will be the prediction layer that classifices the activity
